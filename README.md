...
`c`를 누르면 선택한 세션을 복제합니다. 복제 대상은 지원 provider 중에서 고를 수 있습니다. 같은 에이전트 안에서 복제할 수도 있고, 다른 에이전트 형식으로 변환해 새 세션으로 만들 수도 있습니다.
...
세션을 선택하고 `c`를 누르면 원본과 같은 에이전트 안에 새 세션 ID를 가진 복제본을 만듭니다.

복제는 원본 세션을 수정하지 않고 provider-native 저장 데이터를 직접 복사한 뒤, 새 세션이 충돌 없이 열리도록 세션 ID, cwd, 내부 ID 참조만 패치합니다. 다른 에이전트 형식으로 데이터를 만들고 싶다면 `cokacconvert convert`를 사용합니다.
