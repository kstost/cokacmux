...
`c`를 누르면 선택한 세션의 복제 옵션을 엽니다. 같은 에이전트로는 provider-native 저장 데이터를 복제하고, 다른 에이전트로는 원본 세션을 하나의 context 메시지로 묶은 2-message handoff 세션을 준비합니다.
...
이미 같은 세션의 백그라운드 에이전트가 살아 있으면 새로 실행하지 않고 바로 다시 연결하므로 launch mode 선택을 거치지 않습니다. 다른 cokacmux 프로세스가 이미 붙어 있는 세션은 자동으로 뺏어오지 않습니다.
...
원본 세션의 작업 폴더가 존재하면, 복제 시 실제 폴더 데이터도 함께 보존할지 확인합니다. 동의하면 새 복제 세션과 짝을 이루는 이름으로 `~/.cokacmux/data/` 아래에 작업 폴더 스냅샷을 저장합니다. 이후 그 복제 세션에서 `e`로 에이전트를 시작하려 할 때 저장된 폴더 데이터가 있으면, 원래 작업 경로로 복원할지 다시 확인합니다. 복원 대상 폴더가 이미 있으면 기존 폴더는 같은 부모 경로에 백업한 뒤 스냅샷을 복원합니다. 복원이 성공하면 저장된 스냅샷은 다시 뜨지 않도록 삭제합니다.
...
| `t` | 제목 편집 |
| `r` | 세션 목록 새로고침 |
| `c` | 복제 옵션 열기. 같은 provider는 native clone, 다른 provider는 2-message context handoff |
