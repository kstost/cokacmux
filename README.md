...
> 코딩에이전트를 위해 섬세하게 설계된 Multiplexer, cokacmux.

바이브코딩을 통해 우리는 멋진 코드를 얻습니다. 그런데 놓치지 말아야 할 멋진 것이 하나 더 있습니다. 바로 그 코드를 얻기까지 거쳤던 과정, 즉 세션 안에 담긴 대화 내용입니다.

이 대화는 그 코드를 얻기 위해 사용했던 노하우 그 자체이며, 나중에 스킬셋처럼 재활용할 수 있습니다.

`cokacmux`는 Claude Code, Codex, Pi, OpenCode, GJC 같은 많은 사랑을 받고 있는 코딩 에이전트들이 만들어 내는 여러 세션을 한눈에 관리하고 다시 사용할 수 있는 인터페이스입니다.

코딩에이전트를 위해 섬세하게 설계된 Multiplexer라고 이해하면 쉽습니다. 여러 프로젝트를 동시에 다룰 때도 agent와 terminal을 안전하게 백그라운드에 둔 채 필요한 세션으로 바로 돌아갈 수 있습니다.
...
Claude Code, Codex, Pi, OpenCode, GJC의 작업 세션을 한 화면에서 볼 수 있습니다. 최종 코드뿐 아니라 그 코드를 만들던 대화와 작업 폴더까지 같이 남겨 둡니다.

Codex가 작업 과정에서 만든 non-root 세션은 기본 목록에서 따로 숨겨 최상위 작업과 섞이지 않게 합니다. 앱은 실행할 때마다 `top-level` 범위로 시작하며, `u`를 누르면 현재 실행에서만 전체 세션을 다시 표시할 수 있습니다. `SUB`는 spawned subagent, `REV`는 review, `AUX`는 compact·memory 등 기타 보조 작업, `INT`는 Codex 내부 작업을 뜻합니다. 표식 공간이 5칸 이상이면 `[SUB]`처럼, 3–4칸이면 `SUB`처럼, 1–2칸이면 `S`, `R`, `A`, `I`로 줄여 표시합니다.

현재 범위는 일반 검색과 AI 검색에도 적용됩니다. `u`로 범위를 바꾸면 이전 범위의 검색 결과는 해제됩니다. Codex 저장 형식은 Ultra의 자동 spawn과 명시적인 spawn을 같은 종류로 기록하므로 둘을 더 세분할 수는 없습니다. 판별할 수 없는 세션은 목록에 남으며, 숨긴 세션 데이터는 삭제되거나 변경되지 않습니다.
...
| `u` | Codex non-root 세션을 확인하고 싶을 때 | 현재 실행에서만 top-level 범위와 spawned/review/auxiliary/internal을 포함한 전체 범위를 전환하고 기존 검색 결과를 해제합니다. |
...
이 프로그램을 쓰는 동안 생기는 모든 결과는 사용자가 직접 확인하고 책임져야 합니다. 여기에는 대화 기록 삭제, 파일 삭제, 폴더 복사 실패, 잘못된 폴더에서의 실행, 코딩 도구가 만든 변경, 코딩 도구 사용 비용, 중요한 자료 손실등이 포함됩니다.

중요한 파일이나 대화 기록은 사용 전에 반드시 따로 보관하세요. 이 프로그램의 만든 사람과 기여자는 사용 중 생긴 손해, 비용, 자료 손실, 보안 문제를 비롯한 모든 문제에 대해 책임지지 않습니다.
