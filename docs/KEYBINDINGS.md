...
마우스 포인터가 놓인 패널에서 세로 휠로 스크롤할 수 있습니다. 휠을 돌려도 키보드 포커스는 바뀌지 않습니다.

| 영역 | 휠 동작 |
|---|---|
| 세션 목록 | 한 번에 3개 항목씩 위·아래로 선택 이동. 세션을 실행하거나 연결하지 않음 |
| 미리보기 | 한 번에 3줄씩 스크롤 |
| 실행 중인 에이전트 사이드바 | 목록의 표시 위치만 3줄씩 이동. 활성 에이전트는 변경하지 않음 |
| PTY 패널에서 마우스 모드를 켠 자식 앱 | 패널 내부 좌표로 휠 이벤트 전달. 스크롤 속도·동작은 자식 앱이 결정 |
| 마우스 모드를 켜지 않은 Codex | 기존 transcript 스크롤 키로 3줄 이동 |
| 마우스 모드를 켜지 않은 Claude Code·OpenCode | 기존 내부 스크롤 키로 한 페이지 이동 |
| 일반 터미널 및 그 외 마우스 미지원 앱 | cokacmux의 PTY 출력 기록을 3줄씩 스크롤 |

메인·오른쪽 PTY 패널 모두 지원합니다. 패널 제목·테두리·하단 상태줄에서는 스크롤하지 않습니다. 대화상자나 연결·종료 작업이 진행 중일 때는 뒤쪽 패널로 휠 입력을 전달하지 않으며, 레이아웃이나 연결 대상이 바뀌기 전에 대기열에 들어온 휠 입력도 새 대상에 전달하지 않습니다.

마우스 모드를 켠 자식 앱에는 클릭·버튼 해제·드래그·가로 휠도 전달합니다. 자식 앱의 요청 모드에 맞춰 전송하므로 드래그는 버튼 이동 또는 전체 이동 보고를 요청한 앱에서 동작합니다. 클릭하면 해당 PTY로 키보드 포커스가 이동합니다. 누른 뒤 포인터가 다른 패널로 넘어가도 드래그와 버튼 해제는 처음 누른 PTY에 전달되며 좌표는 그 영역의 가장자리로 제한됩니다. 버튼을 누르지 않은 포인터 이동은 전달하지 않습니다.

화면 크기 변경·포커스 상실·대화상자 전환은 진행 중인 제스처를 원래 누른 위치에서 해제합니다. 연결이 교체되면 이전 제스처를 새 연결에 전달하지 않습니다. 입력 대기열이 찼을 때 버튼 해제는 원래 연결에 보관하고, 그 연결의 다음 입력보다 먼저 처리합니다. 마우스 미지원 자식 앱에는 클릭을 다른 키로 바꾸어 보내지 않습니다.

마우스 캡처가 활성화되므로 터미널 자체의 텍스트 선택에는 해당 터미널의 마우스 캡처 우회 기능이 필요할 수 있습니다. 일부 터미널은 Shift + 마우스를 자체 선택용으로 사용하므로, 자식 앱의 Shift + 드래그 동작에 해당 키가 전달되지 않을 수 있습니다. 종료·오류 복구 시에는 캡처를 해제합니다. 마우스 입력을 제공하지 않는 터미널에서는 기존 키보드 스크롤을 사용하세요.
