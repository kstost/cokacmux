...
클릭·드래그·가로 휠 동작은 추가하지 않았습니다. 마우스 캡처가 활성화되므로 터미널 자체의 텍스트 선택에는 해당 터미널의 마우스 캡처 우회 기능이 필요할 수 있습니다. 종료·오류 복구 시에는 캡처를 해제합니다. 마우스 입력을 제공하지 않는 터미널에서는 기존 키보드 스크롤을 사용하세요.
