...
impl<CB: crate::callbacks::Callbacks> vte::Perform for WrappedScreen<CB> {
    ...
    fn csi_dispatch(&mut self, params: &vte::Params, intermediates: &[u8], _ignore: bool, c: char) {
        // Device reports must be generated at their parse position, not by
        // scanning a completed read chunk (which loses order and duplicates).
        if self.collect_responses && !_ignore && c == 'n' {
            let parameters = params.iter().collect::<Vec<_>>();
            let response = match (intermediates, parameters.as_slice()) {
                ([], [[5]]) => Some(b"\x1b[0n".to_vec()),
                ([], [[6]]) | ([b'?'], [[6]]) => {
                    let (row, col) = self.screen.report_position();
                    let private = if intermediates.is_empty() { "" } else { "?" };
                    Some(format!("\x1b[{}{};{}R", private, row + 1, col + 1).into_bytes())
                }
                _ => None,
            };
