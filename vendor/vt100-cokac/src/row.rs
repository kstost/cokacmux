...
#[serde(try_from = "RowCheckpoint", into = "RowCheckpoint")]
pub struct Row {
    cells: Vec<crate::Cell>,
    wrapped: bool,
}

// Group cells by attributes and concatenate their UTF-8. A normal text row
// needs one attribute record, not a JSON object (and 22-byte array) per cell.
// Length bytes also preserve empty cells and wide-character continuations.
#[derive(serde::Serialize, serde::Deserialize)]
struct RowCheckpoint {
    cols: u16,
    wrapped: bool,
    runs: Vec<(crate::attrs::Attrs, String, Vec<u8>)>,
}

impl From<Row> for RowCheckpoint {
    fn from(row: Row) -> Self {
        let mut runs: Vec<(crate::attrs::Attrs, String, Vec<u8>)> = Vec::new();
        let cols = row.cols();
        for cell in row.cells {
            let attrs = *cell.attrs();
            if runs.last().map_or(true, |run| run.0 != attrs) {
                runs.push((attrs, String::new(), Vec::new()));
            }
            let run = runs.last_mut().unwrap();
            run.1.push_str(cell.contents());
            run.2.push(cell.checkpoint_len());
        }
        Self {
            cols,
            wrapped: row.wrapped,
...
impl TryFrom<RowCheckpoint> for Row {
    type Error = &'static str;

    fn try_from(checkpoint: RowCheckpoint) -> Result<Self, Self::Error> {
        let cols = usize::from(checkpoint.cols);
        if cols == 0 || checkpoint.runs.iter().map(|run| run.2.len()).sum::<usize>() != cols {
            return Err("invalid terminal checkpoint row length");
        }
        let mut cells = Vec::with_capacity(cols);
        for (attrs, text, lengths) in checkpoint.runs {
            let mut offset = 0;
            for len in lengths {
                let end = offset + usize::from(len & 0x1f);
                let contents = text
                    .get(offset..end)
                    .ok_or("invalid terminal checkpoint UTF-8")?;
                cells.push(crate::Cell::from_checkpoint(contents, len, attrs)?);
                offset = end;
            }
            if offset != text.len() {
                return Err("trailing terminal checkpoint cell text");
            }
        }
        for (index, cell) in cells.iter().enumerate() {
            if (cell.is_wide()
                ...
                || (cell.is_wide_continuation() && (index == 0 || !cells[index - 1].is_wide()))
            {
                return Err("invalid terminal checkpoint wide cell");
            }
        }
        Ok(Self {
            cells,
            wrapped: checkpoint.wrapped,
        })
    }
}
...
impl Row {
    ...
    pub(crate) fn checkpoint_bytes(&self) -> usize {
        self.cells.len() * std::mem::size_of::<crate::Cell>()
    }
    ...
    pub fn erase(&mut self, i: u16, attrs: crate::attrs::Attrs) {
        // cokacmux patch: bound-check `i` since callers may pass a column
        // that was valid before a resize but is out of range now.
        let idx = usize::from(i);
        let Some(target) = self.cells.get(idx) else {
            return;
        };
        let wide = target.is_wide();
        self.clear_wide(i);
        self.cells[idx].clear(attrs);
        let cols = self.cols();
        let trim = if wide { 2 } else { 1 };
        if cols >= trim && i == cols - trim {
            self.wrapped = false;
        }
    }
    ...
    pub fn truncate(&mut self, len: u16) {
        ...
        self.wrapped = false;
        // cokacmux patch: when len == 0 the original `cells[len - 1]`
        // underflows to usize::MAX and panics. Skip the wide-cell cleanup
        // in that case.
        if let Some(idx) = usize::from(len).checked_sub(1) {
            if let Some(last_cell) = self.cells.get_mut(idx) {
    ...
    pub fn resize(&mut self, len: u16, cell: crate::Cell) {
        // cokacmux patch: when shrinking we may strand a wide-flagged cell
        // at the new last column whose continuation cell is being dropped.
        // Clear the is_wide flag on the new last cell to keep grid state
        // consistent (the corresponding panic site is clear_wide accessing
        // col + 1).
    ...
    pub fn clear_wide(&mut self, col: u16) {
        // cokacmux patch: bound-check `col` itself and `col +/- 1`. A prior
        // resize can leave a cell marked is_wide whose continuation cell no
        // longer exists -- the original indexing panicked with
        // "index out of bounds: len is N, index is N". Bounds-checking here
        // turns those broken cases into no-ops.
        let Some(cell) = self.cells.get(usize::from(col)) else {
            return;
        };
        let other_idx = if cell.is_wide() {
            let next = usize::from(col).checked_add(1);
            match next {
                Some(i) if i < self.cells.len() => i,
                _ => return,
            }
        } else if cell.is_wide_continuation() {
            let Some(prev) = usize::from(col).checked_sub(1) else {
                return;
            };
            prev
        } else {
        ...
        let attrs = *self.cells[other_idx].attrs();
        self.cells[other_idx].clear(attrs);
    }
