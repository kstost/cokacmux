...
impl Screen {
    pub(crate) fn checkpoint_is_valid(&self) -> bool {
        self.grid.checkpoint_is_valid(false)
            && self
                .alternate_grid
                .checkpoint_is_valid(!self.alternate_screen())
            && self.grid.size() == self.alternate_grid.size()
    }

    pub(crate) fn checkpoint(&self, include_scrollback: bool) -> Self {
        let mut screen = self.clone();
        screen.grid.set_scrollback(0);
        screen.alternate_grid.set_scrollback(0);
        if !include_scrollback {
            screen.grid.clear_checkpoint_history();
            screen.alternate_grid.clear_checkpoint_history();
        }
        screen
    }
...
impl Screen {
    pub(crate) fn text(&mut self, c: char) {
        ...
        if width == 0 {
            ...
            // cokacmux patch: a previous shrink resize can leave the cell
            // at `pos` marked is_wide while its continuation cell at col+1
            // no longer exists. The original unwrap on drawing_cell_mut
            // panicked in that case. Treat the missing continuation as
            // already cleared.
            let prev_was_wide = self
                .grid()
                .drawing_cell(pos)
                .map(|c| c.is_wide())
                .unwrap_or(false);
            ...
            if width > 1 {
                let pos = self.grid().pos();
                // cokacmux patch: same bounds guard as above -- a stale
                // is_wide flag at the new last column (after a shrink
                // resize) used to panic the unwrap on drawing_cell_mut.
                let existing_was_wide = self
                    .grid()
                    .drawing_cell(pos)
                    .map(|c| c.is_wide())
                    .unwrap_or(false);
                if existing_was_wide {
                    ...
                    if let Some(next_next_cell) = self.grid_mut().drawing_cell_mut(next_next_pos) {
                        ...
                        if size.cols > 0 && next_next_pos.col == size.cols - 1 {
