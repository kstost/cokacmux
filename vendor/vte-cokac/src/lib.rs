...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    /// Validate a deserialized cokacmux checkpoint before advancing it.
    /// Serialized parser state is a versioned internal protocol, not terminal input.
    #[cfg(all(feature = "serde", feature = "std"))]
    pub fn checkpoint_is_valid(&self) -> bool {
        if self.intermediate_idx > MAX_INTERMEDIATES
            || self.osc_num_params > MAX_OSC_PARAMS
            || !self.params.checkpoint_is_valid()
            || self.partial_utf8_len >= self.partial_utf8.len()
        {
            return false;
        }
        ...
        self.osc_params[..self.osc_num_params]
            .iter()
